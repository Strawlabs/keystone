          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-body-md font-semibold transition-all whitespace-nowrap cursor-pointer ${
                  isActive
                    ? 'bg-surface-container-lowest text-primary shadow-sm'
                    : 'text-secondary hover:bg-surface-container-lowest/40 hover:text-primary'
                }`}
              >
                {cat === 'all' ? 'All Drawings' : cat.charAt(0).toUpperCase() + cat.slice(1)}
              </button>
            );
          })}
        </div>

        <div className="flex items-center gap-4 px-2 justify-between lg:justify-end">
          ...
          {/* Search Input right aligned inside control row */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {paginatedDrawings.map((d) => {
            ...
            return (
              <div
                key={d.id}
                className="group bg-surface-container-lowest rounded-xl border border-border-subtle overflow-hidden hover:shadow-xl hover:shadow-black/5 transition-all duration-300 flex flex-col h-full shadow-sm"
              >
                {/* Thumbnail Preview */}
                <div className="relative aspect-[4/3] bg-surface-container-high overflow-hidden flex items-center justify-center blueprint-grid">
                  {d.file_url && !isPdf ? (
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      {isPdf && <span className="text-[10px] font-bold text-outline uppercase tracking-wider">PDF Blueprint</span>}
                    </div>
                  )}
                  ...
                  {/* Hover Buttons */}
                  <div className="absolute inset-0 bg-primary/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 backdrop-blur-[2px]">
                    <button
                      onClick={() => {
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex items-start justify-between mb-2 gap-2">
                      ...
                      <span className="text-label-md text-secondary font-semibold shrink-0">
                  ...
                  <div className="pt-3 border-t border-border-subtle flex items-center justify-between mt-auto">
                    ...
                    {statusInfo.text === 'Draft' && canSubmitApproval ? (
                      ...
                      <span className="text-[10px] text-outline font-medium">
                        Uploaded {new Date(d.created_at || Date.now()).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </span>
                    )}
                  </div>
                </div>
          ...
          {canUpload && (
            ...
            >
              ...
              <p className="text-label-md text-secondary max-w-[180px]">
                Drop files here or click to browse CAD blueprints and PDF sheets.
              </p>
            </div>
          )}
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border-subtle">
                {paginatedDrawings.map((d) => {
                  ...
                  const catStyle = CATEGORY_STYLES[d.category] || CATEGORY_STYLES.miscellaneous;
                  return (
                    <tr key={d.id} className="hover:bg-primary/5 transition-colors cursor-pointer group">
                      ...
                      <td className="px-6 py-4 font-semibold text-body-md text-secondary">
                        Rev {d.current_revision || d.revision || 1}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
